...
public final class TcpClientImpl<W, R> extends TcpClient<W, R> {
    ...
    public <WW, RR> TcpClient<WW, RR> addChannelHandlerLast(EventExecutorGroup group, String name,
                                                            Func0<ChannelHandler> handlerFactory) {
        return new TcpClientImpl<>(state.<WW, RR>addChannelHandlerLast(group, name, handlerFactory));
    }
    ...
    public TcpClient<W, R> secure(Func1<ByteBufAllocator, SSLEngine> sslEngineFactory) {
        return copy(((TcpClientState<W, R>)state).secure(sslEngineFactory));
    }

    @Override
    public TcpClient<W, R> secure(SSLEngine sslEngine) {
        return copy(((TcpClientState<W, R>)state).secure(sslEngine));
    }

    @Override
    public TcpClient<W, R> secure(SslCodec sslCodec) {
        return copy(((TcpClientState<W, R>)state).secure(sslCodec));
    }

    @Override
    public TcpClient<W, R> unsafeSecure() {
        return copy(((TcpClientState<W, R>)state).unsafeSecure());
    }
    ...
    public static <W, R> TcpClientImpl<W, R> create(ConnectionProvider<W, R> connectionProvider,
        ...
        return new TcpClientImpl<>(TcpClientState.createTcpState(connectionProvider, epf));
    }

    private static <WW, RR> TcpClientImpl<WW, RR> copy(ClientState<WW, RR> state) {
        return new TcpClientImpl<>(state);
    }
    ...
    public Subscription subscribe(TcpClientEventListener listener) {
        ...
        EventPublisherFactory<TcpClientEventListener> epf =
                (EventPublisherFactory<TcpClientEventListener>) state.getEventPublisherFactory();
        return epf.subscribe(listener);
    }
}
