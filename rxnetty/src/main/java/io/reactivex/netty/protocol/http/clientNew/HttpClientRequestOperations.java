...
interface HttpClientRequestOperations<Input, Owner> {
    ...
    Owner setContentSource(Observable<Input> contentSource);

    <S> Owner setRawContentSource(Observable<S> rawContentSource,
                                  ContentTransformer<S> transformer);

    <S> Owner setRawContent(S content, ContentTransformer<S> transformer);
