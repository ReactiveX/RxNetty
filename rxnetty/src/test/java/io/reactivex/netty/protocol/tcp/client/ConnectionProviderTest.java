...
public class ConnectionProviderTest {
    ...
    public static class ConnectionFactoryRule extends ExternalResource {
        ...
        public <W, R> ConnectionProvider<W, R> start(ConnectionProvider<W, R> toRealize) {
            ...
            ConnectionProvider<W, R> realizedFactory = realize(toRealize);
            realizedFactory.start().subscribe(subscriber);
            subscriber.awaitTerminalEvent();
            subscriber.assertNoErrors();
            return realizedFactory;
        }

        public ConnectionProvider<String, String> realize() {
            return realize(rawFactory);
        }

        public <W, R> ConnectionProvider<W, R> realize(ConnectionProvider<W, R> toRealize) {

            final TcpClientEventPublisher publisher = new TcpClientEventPublisher();
            return toRealize.realize(new ConnectionFactory<W, R>() {
                ...
                public ConnectionObservable<R, W> newConnection(SocketAddress hostAddress) {
                    ...
                    final Connection<R, W> c = ConnectionImpl.create(new EmbeddedChannel(), publisher, publisher);
