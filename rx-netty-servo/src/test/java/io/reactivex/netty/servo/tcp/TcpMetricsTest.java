...
public class TcpMetricsTest {
    ...
    public void testServerMetrics() throws Exception {
        ...
        Assert.assertEquals("Invalid write time metric.", 1, (long) listener.getWriteTimes().getValue());
        ...
        Assert.assertEquals("Invalid flush metric.", 1, (long) listener.getFlushTimes().getValue());
    ...
    private static void doPoolAcquireSuccess(MetricEventsSubject<ClientMetricsEvent<?>> eventsSubject,
        ...
        Assert.assertEquals("Invalid pool acquire times after connection success.", 1, (long)listener.getPoolAcquireTimes().getValue());
    }
}
